...
$sql = ('select * from bancos order by 1');
$query = pg_query($sql);
while ($value = pg_fetch_assoc($query)){
    $button = $button_editar."  ".$button_borrar;
    $array[] = array('codigo' => $value["banco_cod"],'nombre' => $value["banco_nom"],'direccion' => $value["banco_dir"],'telefono' => $value["banco_tel"], 'acciones' => $button);
}
