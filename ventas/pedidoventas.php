...
/*
    $deposito = deposito($ped['item_cod'], $ped['mar_cod'], $ped['ped_cantidad'], $suc);
...
foreach ($pedidoventas as $key => $pedidos) {
    $deposito = deposito($pedidos['item_cod'], $pedidos['mar_cod'], $pedidos['ped_cantidad'], $suc);
...
function deposito($item, $marca, $cant, $suc)
{
    $sqldep = pg_query("SELECT * FROM v_stock WHERE item_cod = $item  AND mar_cod = $marca AND suc_cod = $suc AND stock_cantidad > $cant");
