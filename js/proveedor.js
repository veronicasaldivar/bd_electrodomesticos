...
function refrescarDatos(){
    ...
    //****CODIGO PARA CUANDO HAYA UN API PARA CONSULTAR LOS RUC ****///

    // $("#proveedor").change(function(){
    //     var per = $("#proveedor").val();
    //     alert(`El id de la persona es ${per}`)
    //     rucProveedor(per);
    // });

    // function rucProveedor(per) {
    //     $.ajax({
    //         type: "GET",
    //         url: "prov_tim.php",
    //         data:{per:per}
         ...
         $("#cod_edit").val(cod);
            $.ajax({
                ...
                data: {cod: cod} 
                 ...
                 console.log(dame)
                $("#cod_edit").val(cod);
                $("#proveedor_edit").val(dame.prov_cod).trigger('change');
                $("#ruc_edit").val(dame.prov_ruc);
