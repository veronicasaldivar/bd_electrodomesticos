...
var ordencompras = $('#ordencompras').dataTable( {
   ...
   $('#ordencompras tbody').on( 'click', 'tr td.details-control', function () {        
    ...
    $(document).on("click",".delete",function(){
        ...
        $("#delete").val(nro);
        $(".msg").html('<h4 class="modal-title" id="myModalLabel">Desea eliminar el Registro Nro. '+nro+' ?</h4>');
    });
    $(document).on("click","#delete",function(){
        var cod = $( "#delete" ).val();
        $.ajax({
            type: "POST",
            url: "grabar.php",
            data: {notanro:cod, vencod:0, nrofactura:'', timbrado:0, clicod:0, tipo_nota:'', tipo_mot_nota:'', notamonto:0, desc:'', usu:0, suc:0, detalle:'{}', ope:2}
