...
////FIN TABLA
 ...
 //AGREGAR**
    $(document).on("click","#btnsave",function(){     //#btnsave  es el id del boton  guardar
        ...
        var mod = $("#modelo").val();
        var chapa = $("#chapa").val();
        // console.log(`marca: ${mar}, chapa: ${chapa},modelo: ${mod}`)
         if(chapa!=="" && mod!=='' && mar!==''){ 
              $.ajax({
            type: "POST",
            url: "grabar.php",                
            data: {codigo:0,mar:mar,mod:mod,chapa:chapa,ope:1}
            }).done(function(msg){
           ...
           // $('#sucursal > option[value=""]').attr('selected',false);  //combo
            ...
            $('#marca > option[value=""]').attr('selected',false)
            $("#chapa").val('');
    ...
    $(document).on("click",".editar",function(){
        ...
        $("#lista tbody tr:eq("+pos+")").find('td:eq(0)').each(function () {
            ...
            $.ajax({
                ...
                // console.log(dame);
                $("#cod_edit").val(cod);
                 $('#marca_edit').val(dame.veh_mar_cod).trigger('change'); 
                 $('#modelo_edit').val(dame.veh_mod_cod).trigger('change'); 
            });
        });
    });

    $(document).on("click","#btn_edit",function(){
        var cod,mar,mod,chapa;
       // emp = $("#empresa_edit").val();
        mar = $("#marca_edit").val();
        mod= $("#modelo_edit").val();
        chapa= $("#chapa_edit").val();
        cod = $("#cod_edit").val();
        $("#btn_edit").html("Editando...");
        $.ajax({
            type: "POST",
            url: "grabar.php",
            data: {codigo:cod,mar:mar,mod:mod,chapa:chapa,ope:2}
        }).done(function(msg){
            $('#cerrar').click();
            refrescarDatos();
            humane.log("<span class='fa fa-check'></span> "+msg, { timeout: 4000, clickToClose: true, addnCls: 'humane-flatty-success' });    
 ...
 //INICIO FUNCION DESACTIVAR
    $(document).on("click","#delete",function(){
        ...
        $.ajax({
            ...
            data: {codigo:cod,mar:0,mod:0,chapa:0,ope:3}
...
//ACTIVAR
    ...
    $(document).on("click","#okactive",function(){
        ...
        $.ajax({
            type: "POST",
            url: "grabar.php",
            data: {codigo:cod,mar:0,mod:0,chapa:'',ope:4}
