    ...
    return deta+'<tfoot><tr><th colspan="5" class="text-center" ></th></tr></tfoot></table>\n\
                <div class="row">'+                
                 ...
                 '<div class="col-md-2">' +
                   ...
                   '<a href="../informes/'+Path+'?id='+d.notanro+'/'+d.cod+'/'+d.nro+'/'+d.nro_factura+'" target="_blank" class="btn btn-sm btn-info btn-block" id="print" ><span class="fa fa-print"></span><b> Imprimir</b></a>'+
...
// Insert
    $(document).on("click","#grabar",function(){
        ...
        var suc,usu,proveedor,detalle,notamonto,notanro,timbrado,nrofact,detalle,desc;
        suc = $("#sucursal").val();
        usu = $("#usuario").val();
        ...
        notanro = $("#notanumero").val();
        notamonto = $("#notamonto").val();

        alert(` notanro: ${notanro} proveedor: ${proveedor} timbrado: ${timbrado} nrofact:${nrofact} tipo_nota: ${tipo_nota}  suc:${suc} usu: ${usu} notamonto: ${notamonto} desc: ${desc}`)
        
        if(tipo_nota == 'DEBITO'){
            alert('entro en debito')
            if(notamonto>0 && desc!=="" && proveedor>0 && notanro>0 && timbrado>0 && nrofact !=="" && tipo_nota!==""){
                    $.ajax({
                        //definimos el tipo
                    type: "POST",
                    //la ruta
                    ...
                    //y por ultimo los datos a enviar
                        data: {notanro:notanro,proveedor:proveedor,timbrado:timbrado,nrofact:nrofact,tipo_nota:tipo_nota,suc:suc,usu:usu,notamonto:notamonto,desc:desc, detalle:'{}',ope:1}
                        //--ORDEN: notanro, provcod, provtimbnro, nrofactura, notatipo, usucod, succod, detalle[], operacion
                    }).done(function(msg){
                    mostrarMensajes(msg);
                })
                ...
                if(detalle!=="{}" &&proveedor>0 &&  notanro>0 && nrofact!=="" && timbrado!==""){
                ...
                url: "grabar.php",
                //y por ultimo los datos a enviar
                    data: {notanro:notanro,proveedor:proveedor,timbrado:timbrado,nrofact:nrofact,tipo_nota:tipo_nota,suc:suc,usu:usu, notamonto:0,desc:'',detalle:detalle,ope:1}
                    //--ORDEN: notanro, provcod, provtimbnro, nrofactura, notatipo, usucod, succod, detalle[], operacion
                }).done(function(msg){
                mostrarMensajes(msg);
                    //vaciamos la tabla
