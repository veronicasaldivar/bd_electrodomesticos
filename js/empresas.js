...
//AGREGAR
    ...
    $(document).on("click","#guardar",function(){
     ...
     var email = $("#email").val();
    // var humane = $(); //****////****
    if(emp!=="",ruc!=="",dir !=="",tel!=="",email!==""){
     
       $.ajax({
             url: "grabar.php",
             type: "POST",
             data: {codigo:0,empresa: emp, ruc: ruc,direccion:dir,telefono: tel,email: email, ope:1}
            }).done(function(msg){ 
               // alert(msg);
 ...
 $(document).on("click","#borrar",function(){
      ...
      //var humane = $(); //****////****
        $.ajax({
           type: 'POST',
           url: 'grabar.php',
           data: {codigo: codigo, empresa: '', ruc: '', direccion: '', telefono: '', email: '', ope:3}
        }).done(function(msg){
            $("#cerrar2").click();
            humane.log("<span class='fa fa-check'></span> "+msg, { timeout: 4000, clickToClose: true, addnCls: 'humane-flatty-success' });
            refrescarDatos();
...
$(document).on("click","#btn_edit",function(){
   ...
   $.ajax({
       url: "grabar.php",
       type: "POST",
       data: {codigo: cod, empresa: empresa, ruc: ruc, direccion: direccion, telefono:telefono,  email:email,ope:2}
   }).done(function(msg){
       $('#cerrar').click();
