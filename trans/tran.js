...
//definimos que el campo de texto con id cantidad
 ...
 function format ( d ) {
   ...
   var deta ='<table class="table table-bordered"><tr class="info"><th>Código</th><th>Producto</th><th>Deposito</th><th>Cantidad</th><th>Cant.Recibida</th></tr>';
    //tambien declaramos la variable total y le asignamos un valor de 0
     ...
     for(var x=0;x<d.detalle.length;x++){
        ...
        deta+='<tr>'+
            '<td width=80px>'+d.detalle[x].pro_cod+'</td>'+
            '<td>'+d.detalle[x].pro_desc+'</td>'+
            '<td width=180px>'+d.detalle[x].depo+'</td>'+
            '<td width=180px>'+d.detalle[x].cantidad+'</td>'+
            '<td width=180px>'+d.detalle[x].recib+'</td>'+
        '</tr>';
//total = parseInt(total+d.detalle[x].precio*d.detalle[x].cantidad);
        }
        //y por ultimo retornamos el valor de la variable deta
    return deta+'<tfoot><tr><th colspan="5" class="text-right" >TOTAL:</th></tr></tfoot></table><div class="row">'+                
                '<div class="col-md-3"><strong>Empresa:</strong> '+d.emp+'</div>'+
                '<div class="col-md-3"><strong>Sucursal:</strong> '+d.suc+'</div>'+
   ...
   $("#art").change(function(){
    ...
    $(document).on("click","#cargar",function(){
        ...
        var depo,producto,cantidad,precio;
...
//if (pre<=0) {
  //   humane.log("<span class='fa fa-info'></span> La cantidad debe ser mayor a 0", { timeout: 4000, clickToClose: true, addnCls: 'humane-flatty-warning' });
    //};
        ...
        $("#grilla > tbody:last").append("<tr><td>"+producto+"</td><td>"+desc+"</td><td>"+cantidad+"</td><td>"+recib+
    ...
    $(document).on("click","#guardar",function(){
        ...
        var cod,emp,suc,fun,vehi, envi, entre, ori, des,detalle,depo,en;
        cod = $("#cod").val();
        suc = $("#suc").val();
        emp = $("#emp").val();
        fun = $("#fun").val();
        ...
        $.ajax({
           ...
           data: {cod: cod,suc: suc,  emp: emp,  fun: fun, vehi:vehi, envi:envi, entre:entre, ori:ori, des:des, depo: depo,en:en,  detalle: detalle, ope: 'insercion'}
        }).done(function(msg){
        ...
        $('#ori > option[value=""]').attr('selected',true);
        $('#ori').selectpicker('refresh');
        vaciar();
    ...
    $(document).on("click","#anular",function(){
        ...
        //y realizamos la peticion
                       $.ajax({
                          url: "grabar.php",
                          type: "POST",
                          data: {cod: codigo, suc: '0', emp: '0', fun: '0',vehi: '0',envi: '1111-11-11',entre: '1111-11-11',ori: '0',des: '0', depo:'0', en:'0',detalle: '{}', ope: 'anulacion'}
                       }).done(function(msg){
                           //cerramos el modal,
                           $("#cerrar").click();
