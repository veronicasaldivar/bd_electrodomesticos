...
$apercierre = pg_fetch_all($sql);   
  ...
  //  $cerrar = "<button type='button' class='btn btn-danger confirmar  pull-right' data-toggle='modal' data-target='#confirmacion' data-placement='top'  title='CERRAR CAJA' class='fa fa-times'></button>";
 ...
 while($apercierre = pg_fetch_array($sql)){
    ...
    $rec = "<a target='_blank' class='btn btn-primary btn-circle' href='../informes/imp_recaudacion_dep.php?cod=".$apercierre["aper_cier_cod"]."' title='IMPRIMIR'>REC. DEP<i class='fa fa print'></i></a>";
     ...
     // $cerrar2 = "<button type='button' class='btn btn-round btn-success confirmar pull-right' data-toggle='modal' data-target='#confirmacion2' data-placement='top' title='Cerrar'>CIER<i class='fa fa print'></i></button>";
    ...
    $array[] = array('codigo' => $apercierre["aper_cier_cod"],
        // 'empresa' =>utf8_encode($apercierre["emp_nom"]),
