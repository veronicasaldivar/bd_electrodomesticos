...
<tr width=80px class="success"><th>Nro</th><th>Item</th><th>Marca</th><th>Cantidad</th><th>Deposito O</th><th>Cantidad Recibida</th><th>Depósito D</th></tr>';
    ...
    for (var x = 0; x < d.detalle.length; x++) {
      deta +=
        ...
        "</td>" +
        "<td width=80px>" +
        d.detalle[x].deposito +
        "</td>" +
        "<td width=50px>" +
        d.detalle[x].recibido +
        "</td>" +
        "<td width=80px>" +
        d.detalle[x].depositod +
        "</td>" + //deposito de recepcion
        "</tr>";
    }
    deta +=
      "</tbody>" +
    ...
    return (
      ...
      "?id=" +
      d.nro +
      '" target="_blank" class="btn btn-sm btn-success btn-block" id="print" ><span class="fa fa-print"></span><b> Imprimir</b></a>' +
      "</div>" +
      "</div>"
    );
  }

  //CARGAR GRILLADETALLE
  $(document).on("click", "#cargar", function () {
    ...
    if (stock < cantidad) {
      // si no se dispono la cantidad para el envio
      humane.log(
        "<span class='fa fa-info'></span> LA CANTIDA A SER ENVIADA NO PUEDE SUPERAR LA DISPONIBILIDAD EN STOCK",
      ...
      //si los campos que se destinan a agregar a la grilla estan vacios
      //se mostrara un mensaje de alerta al usuario
      humane.log("<span class='fa fa-info'></span> Complete todos los campos", {
        timeout: 4000,
        clickToClose: true,
  ...
  // ELIMINAR UN ITEM DE LA GRILLA DETALLE
  ...
  $(document).on("click", "#guardar", function () {
    ...
    var usu = $("#usuario").val();
    var feenvio = $("#feenvio").val();
    var feentrega = $("#feentrega").val();
    var vehiculo = $("#vehiculo").val();
    var origen = $("#origen").val();
    var destino = $("#destino").val();
    var depositoo = $("#depositoo").val();
    var depositod = $("#depositod").val();
    ...
    if ($("#en").val() == 1) {
      //ENVIO
      // Verificamos que los campos no esten nulos o vacios para poder enviar los datos al archivos que tiene el sp
      if (
        detalle !== "{}" &&
        cantidad > 0 &&
        origen > 0 &&
        ...
        //si solamente el origen esta vacio
      } else if (origen <= 0) {
        //notificamos al usuario que debe completar el campo
        humane.log("<span class='fa fa-info'></span> Seleccione un origen", {
          timeout: 4000,
          clickToClose: true,
          addnCls: "humane-flatty-warning",
        });
        ...
        //notificamos al usuario que debe completar el campo
        humane.log(
          "<span class='fa fa-info'></span> Verifique los campos por favor!! ",
          {
            timeout: 4000,
            clickToClose: true,
            addnCls: "humane-flatty-warning",
    ...
    if (e.which === 13) {
      ...
      $.ajax({
        ...
        if (msg.trim() != "error") {
          ...
          humane.log(
            "<span class='fa fa-info'></span>  ESTA TRANSFERENCIA NO EXITE O BIEN YA FUE RECEPCIONADA !!!",
            {
              timeout: 6000,
              clickToClose: true,
              addnCls: "humane-flatty-error",
    ...
    if (suc > 0) {
      ...
      $.ajax({
        ...
        if (data != "error") {
          ...
          //SI AUN NO POSEE LA RELACION ITEM- MARCAS
          humane.log(
            "<span class='fa fa-info'></span>  ESTA SUCURSAL NO POSEE DEPOSITOS AUN ",
            {
              timeout: 6000,
              clickToClose: true,
              addnCls: "humane-flatty-error",
    ...
    if (dep > 0) {
      ...
      $.ajax({
        ...
        if (data != "error") {
          ...
          //SI ESTE DEPOSITO NO POSEE ITEMS
          humane.log(
            "<span class='fa fa-info'></span>  ESTE DEPOSITO AUN NO TIENE ITEMS PARA TRANSFERIR ",
            {
              timeout: 6000,
              clickToClose: true,
              addnCls: "humane-flatty-error",
