...
$cn = new conexion();
$cn->conectar();
$sql = pg_query("SELECT * FROM v_rendiciones_fondos_fijos_cab ORDER BY rendicion_fondo_fijo_cod DESC");
...
if (!empty($tabla)) {
    ...
    foreach ($tabla as $key => $cab) {
        $datos['data'][$key]['codigo'] = $cab['rendicion_fondo_fijo_cod'];
        $datos['data'][$key]['ffactura'] = $cab['rendicion_fecha'];
        $datos['data'][$key]['cliente'] = $cab['prov_nombre'];
        $datos['data'][$key]['tipofactcod'] = $cab['tipo_fact_desc'];
        $datos['data'][$key]['cuotas'] = $cab['cuotas'];
        $datos['data'][$key]['estado'] = $cab['rend_estado'];
        $datos['data'][$key]['usuario'] = $cab['usu_name'];
        $datos['data'][$key]['acciones'] = $button;

        $sqldetalle = pg_query('SELECT * FROM  v_rendiciones_fondos_fijos_det WHERE rendicion_fondo_fijo_cod=' . $cab['rendicion_fondo_fijo_cod']);
        $detalles = pg_fetch_all($sqldetalle);

        foreach ($detalles as $key2 => $detalle) {
            $datos['data'][$key]['detalle'][$key2]['rendicion_fondo_fijo_cod'] = $detalle['rendicion_fondo_fijo_cod'];
            ...
            $datos['data'][$key]['detalle'][$key2]['monto'] = $detalle['fact_monto'];
