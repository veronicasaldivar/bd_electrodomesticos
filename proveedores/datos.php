...
$sqlpersonas = pg_query ('select * from v_proveedores  order by 1');
$personas = pg_fetch_all($sqlpersonas);
$datos['data'] = [];

foreach($personas as $key => $personas){
        $datos['data'][$key]['codigo'] = $personas['prov_cod'];
        $datos['data'][$key]['nom'] = $personas['prov_nombre'];
        $datos['data'][$key]['dir'] = $personas['prov_dir'];
        $datos['data'][$key]['tel'] = $personas['prov_tel'];
