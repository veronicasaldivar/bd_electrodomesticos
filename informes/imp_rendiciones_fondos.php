...
$det = array();
$sql1 = ("SELECT * FROM v_rendiciones_fondos_fijos_cab WHERE rendicion_fondo_fijo_cod = $id ");
$result1 = pg_query($sql1);
$cab = pg_fetch_array($result1);


$sql2 = ("SELECT * FROM v_rendiciones_fondos_fijos_det WHERE rendicion_fondo_fijo_cod = $id ");
...
// Tabla simple
    function facturas_varias($cab, $det)
    {
        ...
        $this->Cell(/*1*/90, /*2*/10, /*3*/utf8_decode('Timbrado N.°: '.$cab['fact_tim'].'') , /*4*/'LTR', /*5*/0, /*6*/'C');
        $this->Ln();
        
        // DIRECCION Y FECHA INICION DE VIGENCIA
        $this->Cell(/*1*/90, /*2*/6, /*3*/utf8_decode(" Dirección:  ".$cab['emp_dir']." "), /*4*/'L', /*5*/0, /*6*/'L');  
        $this->SetFont('Times', 'B', 12);
        $this->Cell(/*1*/90, /*2*/4, /*3*/utf8_decode(' RENDICION FONDO FIJO ') , /*4*/'LR', /*5*/0, /*6*/'C');
        $this->SetFont('Times', '', 10);
        $this->Ln();

        // TELEFONO Y FECHA FIN DE VIGENCIA
        $this->Cell(/*1*/90, /*2*/8, /*3*/utf8_decode(" Teléfono:  ".$cab['emp_tel']." "), /*4*/'L', /*5*/0, /*6*/'L'); 
        $this->Cell(/*1*/90, /*2*/6, /*3*/utf8_decode('Fecha vigencia hasta: '.$cab['fact_tim_vig_hast'].' ') , /*4*/'LR', /*5*/0, /*6*/'C');
        $this->Ln();
        ...
        $this->Cell(/*1*/90, /*2*/6, /*3*/utf8_decode($cab['fact_nro']), /*4*/'LBR', /*5*/0, /*6*/'C');
        $this->Ln();

        //FECHA DE EMISION Y CONDICION DE VENTA
        $this->Cell(/*1*/90, /*2*/8, /*3*/utf8_decode("  Fecha de la Factura: ".$cab['fact_fecha']), /*4*/'LBR', /*5*/0, /*6*/'L');
        ...
        foreach($det as $row)
        {
            ...
            $this->Cell(/*1*/95, /*2*/6, /*3*/utf8_decode($row['rubro_desc']), /*4*/1, /*5*/0, /*6*/'L');
              //  $this->SetFont('Times', '', 10);
            $this->Cell(/*1*/15, /*2*/6, /*3*/number_format($row["fact_monto"],0,',','.'), /*4*/1, /*5*/0, /*6*/'R');
