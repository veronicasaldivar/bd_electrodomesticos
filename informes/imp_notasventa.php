...
$det = array();
$sql1 = ("SELECT * from v_notas_ventas_cab where nota_ven_cod = ' $id' ");
$result1 = pg_query($sql1);
$cab = pg_fetch_array($result1);
$sql2 = ("SELECT * from v_notas_ventas_detalles where nota_ven_cod = ' $id' ");
...
class PDF extends FPDF
{
    ...
    function notascompras($cab, $det)
    {
        ...
        $this->Cell(/*1*/90, /*2*/ 10, /*3*/ utf8_decode('NOTA DE ' . $cab['nota_ven_tipo'] . ''), /*4*/ 'LTR', /*5*/ 0, /*6*/ 'C');
        $this->Ln();
        ...
        $this->Cell(/*1*/90, /*2*/ 5, /*3*/ utf8_decode('Nota N.°:  ' . $cab['nota_ven_cod'] . ''), /*4*/ 'LR', /*5*/ 0, /*6*/ 'L');
        $this->Ln();
        ...
        $this->Cell(/*1*/55, /*2*/ 8, /*3*/ utf8_decode($cab['nota_ven_fecha']), /*4*/ 'B', /*5*/ 0, /*6*/ 'L');

        $this->Cell(/*1*/90, /*2*/ 8, /*3*/ utf8_decode('Estado:  ' . $cab['nota_ven_estado'] . ''), /*4*/ 'BR', /*5*/ 0, /*6*/ 'L');
        $this->Ln();

        // NOMBRE O RAZON SOCIAL Y RUC DEL PROVEEDOR
        $this->Cell(/*1*/90, /*2*/ 8, /*3*/ utf8_decode('Nombre o Razón Social:  ' . $cab['cli_nom'] . ''), /*4*/ 'L', /*5*/ 0, /*6*/ 'L');
        $this->Cell(/*1*/90, /*2*/ 8, /*3*/ utf8_decode('RUC: ' . $cab['cli_ruc'] . ''), /*4*/ 'R', /*5*/ 0, /*6*/ 'L');
        $this->Ln();

        // TIMBRADO Y NUMERO DE FACTURA
        $this->Cell(/*1*/90, /*2*/ 8, /*3*/ utf8_decode('Timbrado: ' . $cab['timb_nro'] . ''), /*4*/ 'L', /*5*/ 0, /*6*/ 'L');
        $this->Cell(/*1*/90, /*2*/ 8, /*3*/ utf8_decode('Factura N.°: ' . $cab['nota_ven_nro_fact'] . ''), /*4*/ 'R', /*5*/ 0, /*6*/ 'L');
        $this->Ln();

        if ($cab['nota_ven_tipo'] == 'CREDITO' && $cab['nota_ven_motivo'] == 'DEVOLUCION') {
            ...
            $total = 0;
            foreach ($det as $row) {
                $subtotal = $row["nota_ven_cant"] * $row["nota_ven_precio"];
                $total = $total + $subtotal;
                $this->Cell(/*1*/15, /*2*/ 6, /*3*/ utf8_decode($row['item_cod']), /*4*/ 1, /*5*/ 0, /*6*/ 'C');
                $this->Cell(/*1*/15, /*2*/ 6, /*3*/ utf8_decode($row['nota_ven_cant']), /*4*/ 1, /*5*/ 0, /*6*/ 'R');
                $this->SetFont('Times', '', 8); //TIPO DE LETRA PARA SUBTITULO
                $this->Cell(/*1*/100, /*2*/ 6, /*3*/ utf8_decode($row['item_desc']), /*4*/ 1, /*5*/ 0, /*6*/ 'L');
                $this->SetFont('Times', '', 10); //TIPO DE LETRA PARA SUBTITULO
                $this->Cell(/*1*/25, /*2*/ 6, /*3*/ number_format($row["nota_ven_precio"], 0, ',', '.'), /*4*/ 1, /*5*/ 0, /*6*/ 'R');
            ...
            $this->Ln();
        } else if ($cab['nota_ven_tipo'] == 'DEBITO') {
