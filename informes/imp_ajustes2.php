...
$sqlCab = ("SELECT * FROM v_ajustes_cab WHERE ajus_cod = $id ORDER BY ajus_cod");
$resultCab = pg_query($con, $sqlCab);
$cab = pg_fetch_array($resultCab);

$sqlDep = ("SELECT * FROM v_ajustes_det WHERE ajus_cod = $id LIMIT 1");
$resDep = pg_query($con, $sqlDep);
$dep = pg_fetch_assoc($resDep);

$sqlDet = ("SELECT * FROM v_ajustes_det WHERE ajus_cod = $id ");
$resultDet = pg_query($con, $sqlDet);
$det = array();
while ($row = pg_fetch_array($resultDet)) {
    $det[] = $row;
}
...
{
// Tabla simple
    function presupuestos_cliente($cab, $det, $dep)
    {
...
$pdf->AddPage();
$pdf->presupuestos_cliente($cab, $det, $dep);
$pdf->Output();
?>
