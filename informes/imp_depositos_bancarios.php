...
$sql = ("SELECT * FROM v_boletas_depositos  WHERE movimiento_nro = ".$id."");
$result = pg_query($sql);
...
// Tabla simple
    function presupuestos_cliente($cab)
    {
        ...
        $this->Cell(/*1*/90, /*2*/10, /*3*/'DEPOSITO BANCARIOS', /*4*/'LTR', /*5*/0, /*6*/'C');
        $this->Ln();
        ...
        $this->Cell(/*1*/90, /*2*/5, /*3*/utf8_decode('Movimiento N.°: '.$cab['movimiento_nro'].''), /*4*/'LR', /*5*/0, /*6*/'L');
        $this->Ln();
        ...
        $this->Cell(/*1*/180, /*2*/8, /*3*/utf8_decode('Fecha de Operacion:  '.$cab['fecha_deposito'].''), /*4*/'LBR', /*5*/0, /*6*/'L');
        $this->Ln();
        
        // NOMBRE Y APELLIDO
        $this->Cell(/*1*/180, /*2*/8, /*3*/utf8_decode('Nombre y Apellido del Usuario:  '.$cab['fun_nom'].''), /*4*/'LR', /*5*/0, /*6*/'L');
        $this->Ln();

        // ENTIDADES
        $this->Cell(/*1*/180, /*2*/8, /*3*/utf8_decode('Entidad:  '.$cab['ent_nom'].'    Cuenta Corriente N°:  '.$cab['cuenta_corriente_nro']), /*4*/'LR', /*5*/0, /*6*/'L');
        $this->Ln();

        // TIPO RECLAMO / SUGERENCIAS y ESTADO
        $this->Cell(/*1*/90, /*2*/8, /*3*/utf8_decode('Monto Depositado: '.$cab['monto'].''), /*4*/'LB', /*5*/0, /*6*/'L');
        $this->Cell(/*1*/90, /*2*/8, /*3*/utf8_decode('Estado: '.$cab['estado'].''), /*4*/'BR', /*5*/0, /*6*/'L');
        $this->Ln();
     }
}
$pdf = new PDF();
