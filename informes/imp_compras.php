...
$det = array();
$sql1 = ("SELECT * FROM v_compras_cab WHERE comp_cod = $id ");
$result1 = pg_query($sql1);
$cab = pg_fetch_array($result1);
$sql2 = ("SELECT * FROM v_compras_det WHERE comp_cod = $id ");
...
// Tabla simple
    function compras($cab, $det)
    {
        ...
        $this->Cell(/*1*/90, /*2*/6, /*3*/utf8_decode('Fecha Fin Vigencia: '.$cab['prov_timb_vig'].'') , /*4*/'LR', /*5*/0, /*6*/'C');
        $this->Ln();
        ...
        foreach($det as $row)
        {
            $subtotal = $row['comp_costo'] * $row['comp_cantidad'];
            ...
            $this->Cell(/*1*/95, /*2*/6, /*3*/utf8_decode($row['item_desc'].' '.$row['mar_desc']), /*4*/1, /*5*/0, /*6*/'L');
              //  $this->SetFont('Times', '', 10);
            $this->Cell(/*1*/15, /*2*/6, /*3*/number_format($row["comp_costo"],0,',','.'), /*4*/1, /*5*/0, /*6*/'R');
