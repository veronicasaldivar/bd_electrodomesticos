<?php

require "../funciones/fpdf/fpdf.php";

require "../clases/conexion.php";
$id = $_GET["id"];
// echo $id;
$id2 = explode('/',$id);
// $id2  = $id.str_split('/');
// print_r($id2);
$con = new conexion();
$con->conectar();

$det = array();
// prov_cod = '$id2[0]' and prov_timb_nro = '$id2[1]' 
$sql1 = ("select * from v_compras_cab where nro_factura = '$id2[2]' and prov_cod = '$id2[0]' and prov_timb_nro = '$id2[1]'  ");
$result1 = pg_query($sql1);
$cab = pg_fetch_array($result1);
$sql2 = ("select * from v_compras_det where nro_factura = '$id2[2]' and prov_cod = '$id2[0]' and prov_timb_nro = '$id2[1]'  ");
$result2 = pg_query($sql2);
while ($row = pg_fetch_array($result2)) {
    $det[] = $row;
}
$con->destruir();
class PDF extends FPDF
{
// Tabla simple
    function compras($cab,$det)
    {
        // $this->Image('../img/INFORMES.JPG',60,11,80,25,'JPG');
        $this->Cell(165,30,"");
        $this->Ln();
        $this->Cell(165,10,"Detalles de Compras");
        $this->Ln();

        $this->Cell(25,6,utf8_decode('Nro. Factura: '));
        $this->Cell(33,6,$cab["nro_factura"]);
        $this->Cell(15,6,utf8_decode('Estado: '));
        $this->Cell(30,6,$cab["comp_estado"]);
        $this->Cell(15,6,utf8_decode('Fecha: '));
        $this->Cell(30,6,$cab["comp_fecha"]);
        $this->Ln();
        $this->Cell(25,6,utf8_decode('Tipo Factura: '));
        $this->Cell(30,6,$cab["tipo_fact_desc"]);
        $this->Cell(15,6,utf8_decode('Plazo: '));
        $this->Cell(30,6,$cab["comp_plazo"]);
        $this->Cell(15,6,utf8_decode('Cuotas: '));
        $this->Cell(30,6,$cab["comp_cuotas"]);
         $this->Ln();
        $this->Cell(25,6,utf8_decode('Funcionario: '));
        $this->Cell(45,6,$cab["fun_nom"]);
        $this->Cell(25,6,utf8_decode('Proveedor: '));
        $this->Cell(30,6,$cab["prov_nombre"]);
        $this->Ln();
        // $this->Cell(18,6,utf8_decode('Empresa: '));
        // $this->Cell(20,6,$cab["emp_nom"]);
        // $this->Cell(9,6,utf8_decode('RUC: '));
        // $this->Cell(23,6,$cab["emp_ruc"]);
        // $this->Cell(18,6,utf8_decode('Direccion: '));
        // $this->Cell(60,6,utf8_decode($cab["emp_dir"]));
        //     $this->Ln();
        // $this->Cell(20,6,utf8_decode('Email: '));
        // $this->Cell(50,6,utf8_decode($cab["emp_email"]));

       // $this->Ln();
       // $this->Ln();
        $this->Ln();
        //Titulo
        $header = array('Codigo','Cantidad','Descripcion','Precio Unitario','Subtotal');
        // Cabecera
        $this->Cell(14,7,$header[0],1);
        $this->Cell(20,7,$header[1],1);
        $this->Cell(100,7,$header[2],1);
        $this->Cell(25,7,$header[3],1);
        $this->Cell(25,7,$header[4],1);
        $this->Ln();
        // Datos
        $total = 0;
        foreach($det as $row)      // var tipoiva = $('#tipoiva').val();
                                     // var ivatotal = subtotal * tipoiva / 100;
        {
            $subtotal = $row["comp_cantidad"] * $row["comp_precio"];
            $total = $total + $subtotal;
            $this->Cell(14,6,$row["item_cod"],2);
            $this->Cell(20,6,$row["comp_cantidad"],2);
            $this->Cell(100,6,$row["item_desc"],2);
            $this->Cell(25,6,number_format($row["comp_precio"],0,',','.'),2);
            $this->Cell(25,6,number_format($subtotal,0,',','.'),2);
            $this->Ln();
        }
        $this->Cell(159,6,'Totales',1);
        $this->Cell(25,6,number_format($total,0,',','.'),1);
        $this->Ln();
    }
}
$pdf = new PDF();
// Carga de datos
$pdf->SetFont('Arial','',10);
$pdf->AddPage();
$pdf->compras($cab,$det);
$pdf->Output();
?>