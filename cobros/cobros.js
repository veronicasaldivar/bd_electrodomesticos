...
<tr width=90px class="success"><th>Venta N.°</th><th>Cuotas</th><th>Vencimiento</th><th>Monto</th><th>Saldo</th><th>Estado</th><th>Fecha cobro</th></tr>';
  ...
  $(document).on("click", ".agregarCobroTarjetasSolo", function () {
    ...
    if (todoMonTarj > 0 && todoNumTarj > 0 && todoCodAut > 0) {
      ...
      if (totalAcumulado <= todoMontoPagar) {
              ...
              .each(function (col1) {
                if (col1 === 1) {
                  ...
                  if (co === todoMarTarjCod) {
                    // repetido = true;
                    $("#grilladetalletarjetasSolo tbody tr:eq(" + filac + ")")
                      .children("td")
                      .each(function (col2) {
                        if (col2 === 2) {
                          co2 = $(this).text();
                          if (co === todoMarTarjCod && co2 === todoNumTarj) {
                            $(
                              "#grilladetalletarjetasSolo tbody tr:eq(" +
                                filac +
                                ")"
                            )
                              .children("td")
                              .each(function (col3) {
                                if (col3 === 3) {
                                  ...
                                  if (
                                    ...
                                    //  si coincide marca, num tarj y autori.
                                    repetido = true;
                                    humane.log(
                                      "<span class='fa fa-info'></span> YA SE HA CARGADO ESTA TARJETA CON ESTE COD. DE  AUTIRAZACION",
                                      {
                                        timeout: 4000,
                                        clickToClose: true,
                                        addnCls: "humane-flatty-warning",
        ...
        if (!repetido) {
          $("#grilladetalletarjetasSolo > tbody:last").append(
            '<tr><td style="text-align: center;">' +
              ...
              todoMonTarj +
              '</td><td class="eliminarDetalleTarjetasSolo"><input type="button" value="Х" id="bf"   class="bf"  style="background:  pink; color: black;"/></td></tr>'
          );
          contador++;
        }
      ...
      humane.log(
        "<span class='fa fa-info'></span> Por favor complete todos los campos de la tarjeta",
        { timeout: 4000, clickToClose: true, addnCls: "humane-flatty-warning" }
      );
    }
  });
    ...
    $("#grilladetalletarjetasSolo tbody tr").each(function (fila) {
      $(this)
        ...
        .each(function (col) {
          if (col === 6) {
            totalTarjetas =
              totalTarjetas + parseInt($(this).text().replace(/\./g, ""));
          }
        });
    });
  ...
  function calcularTotalSolo() {
    ...
    if (totalDeuda >= totalPagar) {
      ...
      humane.log(
        "<span class='fa fa-info'></span> EL MONTO A PAGAR NO PUEDE SER MAYOR AL MONTO DE LA DEUDA2",
        { timeout: 4000, clickToClose: true, addnCls: "humane-flatty-warning" }
      );
    }
  }
                      ...
                      .each(function (col2) {
                        if (col2 === 2) {
                          ...
                          if (co === todoNumCuen && co2 === todoSerie) {
                            $(
                              "#grilladetallechequesSolo tbody tr:eq(" +
                                filac +
                                ")"
                            )
                              .children("td")
                              .each(function (col3) {
                                if (col3 === 3) {
                                  ...
                                  if (
                                    ...
                                    repetido = true;
                                    humane.log(
                                      "<span class='fa fa-info'></span> ESTE NUMERO CHEQUE YA HA SIDO CARGADO",
                                      {
                                        timeout: 4000,
                                        clickToClose: true,
                                        addnCls: "humane-flatty-warning",
  ...
  $(document).on("click", ".agregarCobroTarjetas", function () {
    ...
    if (todoMonTarj > 0 && todoNumTarj > 0 && todoCodAut > 0) {
      ...
      if (totalAcumulado <= todoMontoPagar) {
            ...
            $(this)
              ...
              .each(function (col1) {
                if (col1 === 1) {
                  ...
                  if (co === todoMarTarjCod) {
                    // repetido = true;
                    $("#grilladetalletarjetas tbody tr:eq(" + filac + ")")
                      .children("td")
                      .each(function (col2) {
                        if (col2 === 2) {
                          co2 = $(this).text();
                          if (co === todoMarTarjCod && co2 === todoNumTarj) {
                            $(
                              "#grilladetalletarjetas tbody tr:eq(" +
                                filac +
                                ")"
                            )
                              .children("td")
        ...
        if (!repetido) {
          $("#grilladetalletarjetas > tbody:last").append(
            '<tr><td style="text-align: center;">' +
              cobroCod +
              '</td><td style="text-align: center;">' +
              todoMarTarj +
              ...
              '</td><td style="text-align: right;">' +
              todoMonTarj +
              '</td><td class="eliminarDetalleTarjetas"><input type="button" value="Х" id="bf"   class="bf"  style="background:  pink; color: black;"/></td></tr>'
          );
          contador++;
        }
      ...
      humane.log(
        "<span class='fa fa-info'></span> Por favor complete todos los campos tarjetas",
        { timeout: 4000, clickToClose: true, addnCls: "humane-flatty-warning" }
      );
    }
  });
  ...
  $(document).on("click", ".agregarCobroCheques", function () {
    ...
    if (
      ...
      if (totalAcumulado <= todoMontoPagar) {
            ...
            $(this)
              ...
              .each(function (col1) {
                if (col1 === 1) {
                  ...
                  if (co === todoNumCuen) {
                    // repetido = true;
                    $("#grilladetallecheques tbody tr:eq(" + filac + ")")
                      .children("td")
                      .each(function (col2) {
                        if (col2 === 2) {
                          co2 = $(this).text();
        ...
        if (!repetido) {
          $("#grilladetallecheques > tbody:last").append(
            '<tr><td style="text-align: center;">' +
              cobroCod +
              '</td><td style="text-align: center;">' +
              todoNumCuen +
              ...
              '</td><td style="text-align: right;">' +
              todoTipCheq +
              '</td><td class="eliminarDetalleCheques"><input type="button" value="Х" id="bf"   class="bf"  style="background:  pink; color: black;"/></td></tr>'
          );
          contador++;
        }
        cargarGrillaCheques();
    ...
    if (cliente > 0) {
      $.ajax({
        ...
        if (data != "error") {
          ...
          humane.log(
            "<span class='fa fa-info'></span>  ESTE CLIENTE NO POSEE UNA VENTA CON ESTADO PENDIENTE ",
            {
              timeout: 6000,
              clickToClose: true,
  ...
  function limpiarCampos() {
    ...
    $("#todoMarcaTarjeta").val("0").trigger("change");
    $("#todoNumeroTarjeta").val("");
    $("#todoCodigoAutorizacion").val("");
    $("#todoEntidadEmisora").val("0").trigger("change");
    $("#todoEntidadAdherida").val("0").trigger("change");
    $("#todoMontoTarjeta").val("");
    $("#grilladetalletarjetas tbody tr").remove();
    $("#totalTarjetas").html('Subtotal Tarjetas: 0 Gs.');
    $("#todoNumeroCuenta").val("");
    $("#todoSerie").val("");
    $("#todoNumeroCheque").val("");
    $("#todoChequeMonto").val("");
    $("#todoChequeEmision").val("");
    $("#todoLibrador").val("");
    $("#todoBancos").val("0").trigger("change");
    $("#todoTipoCheques").val("0").trigger("change");
    $("#grilladetallecheques tbody tr").remove();
    $("#totalCheques").html("SUBTOTAL CHEQUE: 0 Gs.");
    //fin todos

    // tarjetas solo
    $("#tarjetaMarjSolo").val("0").trigger("change");
    $("#tarjetaNumSolo").val("");
    $("#tarjetaCodAutSolo").val("");
    $("#tarjetaEntEmiSolo").val("0").trigger("change");
    $("#tarjetaEntAdhSolo").val("0").trigger("change");
    $("#tarjetaMonTarjSolo").val("");
    $("#grilladetalletarjetasSolo tbody tr").remove();
    $("#totalTarjetasSolo").html("SUBTOTAL TARJETAS: 0 Gs.");
    // fin tarjetas solo
